...
class Extension {
    _loginManagerCanHibernate(asyncCallback) {
        if (Prefs.getHibernateOptionEnabled() && this._loginManager._proxy) {
            // systemd path
    ...
    _onHibernateClicked() {
        ...
        if (Prefs.getHibernateConfirmationOptionEnabled()) {
            this._dialog = new ConfirmDialog.ConfirmDialog(
                ConfirmDialog.HibernateDialogContent
            );
    ...
    _cancelDisableExtension(notAgain) {
        if (notAgain) Prefs.setHibernateWorksCheckEnabled(false);
    }

    _checkRequirements() {
        if (!LoginManager.haveSystemd()) {
            this._dialog = new ConfirmDialog.ConfirmDialog(
                ConfirmDialog.SystemdMissingDialogContent
            );
    ...
    _checkDidHibernate() {
        ...
        // hibernate failed
        this._dialog = new ConfirmDialog.ConfirmDialog(
            ConfirmDialog.HibernateFailedDialogContent
        );
        this._dialog.connect("DisableExtension", this._disableExtension);
